...
# Турнир агентов за одним столом (главный показатель успеха)

`table_play` сажает 2..6 политик за один стол со **случайной раздачей** и
измеряет винрейт в **BB/100** с 95% доверительным интервалом. Стек каждого
сбрасывается к 100 BB каждую раздачу (chip-EV), реализованы корректные побочные
банки. Игра нулевая → сумма BB/100 всех игроков ≈ 0 (проверка корректности).

Типы мест (`--seat`):
- `model:FILE` — обученный агент (веса из train_pokerbench)
- `random` — случайные легальные ходы
- `call` — calling station (всегда чек/колл)
- `tight` — нит (фолд против ставки на слабой руке)
- `aggro` — агрессор

Запуск:
```bash
bash tools/arena.sh                       # твой агент vs call/random/tight, 100k раздач
# или вручную:
./bin/table_play --seat model:agent_weights.bin --seat call --seat random --seat tight \
                 --hands 200000 --seed 7
# дуэль двух моделей:
./bin/table_play --seat model:v1.bin --seat model:v2.bin --hands 300000
```
Чем больше `--hands`, тем уже доверительный интервал. Если BB/100 модели уверенно
выше нуля и выше ботов (с учётом ±CI) — обучение удалось.
